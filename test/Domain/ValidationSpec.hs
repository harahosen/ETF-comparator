  ...
  describe "Domain.Validation" $ do
    describe "validateRawETF" $ do
      ...
      it "rejects ETF with negative weights" $ do
        let etf = RawETF (FundId "TEST")
              [ Holding (RawAssetId "AAPL") (Just (CanonicalAssetId "AAPL")) Nothing (Weight (-0.1))
              , Holding (RawAssetId "MSFT") (Just (CanonicalAssetId "MSFT")) Nothing (Weight 1.1)
              ]
      ...
      it "validates ETF with holdings that sum to more than 1" $ do
        let etf = RawETF (FundId "TEST")
              [ Holding (RawAssetId "AAPL") (Just (CanonicalAssetId "AAPL")) Nothing (Weight 0.6)
              , Holding (RawAssetId "MSFT") (Just (CanonicalAssetId "MSFT")) Nothing (Weight 0.6)
              ]
        validateRawETF etf `shouldBe` Right etf

      it "collects all validation errors" $ do
        let etf = RawETF (FundId "TEST")
              [ Holding (RawAssetId "AAPL") (Just (CanonicalAssetId "AAPL")) Nothing (Weight (-0.5))
              , Holding (RawAssetId "MSFT") (Just (CanonicalAssetId "MSFT")) Nothing (Weight (-0.3))
              , Holding (RawAssetId "AAPL") (Just (CanonicalAssetId "AAPL")) Nothing (Weight 0.2)
              ]
        case validateRawETF etf of
